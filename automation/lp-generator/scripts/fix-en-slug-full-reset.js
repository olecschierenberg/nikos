 ...
 *
 * Update 2026-09-09: Der Dry-Run zeigt, dass die Zeile inzwischen aktiv="x"
 * hat -- die Seite ist also unter dem falschen Slug LIVE gegangen (bestaetigt
 * unter https://nikos.info/en/lp/besucherlenkung-sail-amsterdam-2030/). Der
 * Sicherheits-Check unten wuerde das Skript deshalb normalerweise abbrechen
 * lassen. Mit --force wird dieser Check bewusst uebersprungen, weil wir genau
 * diesen Fall (bereits live, aber mit falschem Slug) beheben wollen.
 *
 * Aufruf: node fix-en-slug-full-reset.js (Dry-Run) / --live (schreibt wirklich)
 *         --force zusaetzlich, um den aktiv-Sicherheitscheck zu ueberspringen
 */
...
async function main() {
  ...
  if (String(j.aktiv || '').trim() !== '' && !FORCE) {
    log(`ABBRUCH: aktiv="${j.aktiv}" ist nicht leer -- Zeile ist offenbar schon (wieder) live oder in einem anderen Zustand, nichts angefasst. (Mit --force ueberschreiben, falls das gewollt ist.)`);
    return;
  }
  if (String(j.aktiv || '').trim() !== '' && FORCE) {
    log(`HINWEIS: aktiv="${j.aktiv}" ist nicht leer, aber --force gesetzt -- setze trotzdem komplett zurueck (Seite ist unter falschem Slug live).`);
  }
