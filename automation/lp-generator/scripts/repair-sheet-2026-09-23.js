...
(async () => {
  ...
  if (dupes.length !== 2) fail(`erwartet 2 Sindelfingen-Kopien, gefunden ${dupes.length}.`);
  ...
  const sailTargets = [];
  for (const s of SAIL) {
    if (rows.some((r) => n(r.slug) === s.slug)) fail(`slug ${s.slug} steht bereits im Sheet -- Reparatur nicht noetig/unklar.`);
  ...
  if (LIVE) {
    ...
    log(`Kontrolle: Sindelfingen-Kopien=${d} (soll 0), Sail-Zeilen=${s.join('/')} (soll 1/1)`);
    if (d !== 0 || s.some((c) => c !== 1)) fail('Kontrolle fehlgeschlagen.');
  }
  log('FERTIG.');
